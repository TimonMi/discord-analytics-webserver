...
if (index == -1) {
  for (var i = 0; i < scrapeList.length; i++) {
    select.insertAdjacentHTML(
      'beforeend',
      '<option>' + decodeURI(scrapeList[i]) + '</option>'
    );
  }
...
table.innerHTML =
  '<tr><th>Server name</th><td><i>' +
  server_name +
  '</i></td></tr> <tr><th>Number of messages</th><td><i>' +
  number_of_messages +
  '</i></td></tr> <tr><th>Number of words</th><td><i>' +
  number_of_words +
  '</i></td></tr> <tr><th>Words per message ratio</th><td><i>' +
  words_per_message_ratio +
  '</i></td></tr> <tr><th>Number of users</th><td><i>' +
  number_of_users +
  '</i></td></tr> <tr><th>Number of channels</th><td><i>' +
  number_of_channels +
  '</i></td></tr> <tr><th>First message</th><td><i>' +
  first_message +
  '</i></td></tr> <tr><th>Last message</th><td><i>' +
  last_message +
  '</i></td></tr>';
...
Object.keys(discord_data.users).forEach(function(name) {
  select.insertAdjacentHTML(
    'beforeend',
    `<a onclick="reloadName(this.innerHTML)">` + name + '</a>'
  );
});
...
function user_tables_construct(api_response) {
  const createdTimestamp =
    new Date(api_response['createdTimestamp'])
      .toISOString()
      .slice(0, 19)
      .replace('T', ' ') + ' UTC';
  ...
  table_avatar.innerHTML = `<tr><th align="left"><img id="avatar" src="${
  ...
  table_name.innerHTML =
    '<tr><th>Account created at</th><td><i>' +
    createdTimestamp +
    '</i></td></tr> <tr><th>First message</th><td><i>' +
    User['firstMessage'] +
    '</i></td></tr> <tr><th>Last message</th><td><i>' +
    User['lastMessage'] +
    '</i></td></tr> <tr><th>Message count</th><td><i>' +
    User['messagesCount'] +
    '</i></td></tr> <tr><th>Word count</th><td><i>' +
    User['wordsCount'] +
    '</i></td></tr> <tr><th>Words per message ratio</th><td><i>' +
    User['words_per_message_ratio'] +
    '</i></td></tr> <tr><th>Image count</th><td><i>' +
    User['imagesCount'] +
    '</i></td></tr> <tr><th>Most active channel</th><td><i>' +
    `#${User['mostActiveChannel'][0][0]} (${
      User['mostActiveChannel'][0][1]
    } msg)` +
    '</i></td></tr> <tr><th>Most used word</th><td><i>' +
    `${mostUsedWord[0][0]} (${mostUsedWord[0][1]} times)` +
    '</i></td></tr> <tr><th>Most active day</th><td><i>' +
    `${User['history'][0][0]} (${User['history'][0][1]} msg)` +
    '</i></td></tr>';
    ...
    function load_loop(images_queue) {
      if (images_queue.length != 0) {
        ...
        for (var i = 0; i < load_ammount; i++) {
          try {
            const index = shift + i;
            images_table.insertAdjacentHTML(
              'beforeend',
              `<img data-toggle="tooltip" title="${new Date(
                images_queue[index][0]
              )
                .toISOString()
                .slice(0, 19)
                .replace('T', ' ') +
                ' UTC'}" class="pop-over lozad" data-src="${
                images_queue[index][1]
              }"/>`
            );
          } catch (error) {
            break;
        ...
        if (shift < images_queue.length) {
          ...
          document.getElementById(
            'image_count'
          ).innerHTML = `${shift.toString()}/${images_queue.length.toString()}`;
        } else {
          console.log('end');
          ...
          document.getElementById(
            'image_count'
          ).innerHTML = `${images_queue.length.toString()}/${images_queue.length.toString()}`;
          update_pop_over();
          observer.observe();
        }
      } else {
...
if (api_response.response.code == 200) {
  ...
  table_avatar.innerHTML = `<p align="center">User <b style="font-size: 1rem !important">${
    Names[0]
  }</b> <i>(${User['id']})</i> is unreachable</p>`;
  table_name.innerHTML = '';
  images_table.innerHTML = '';
}
...
function reloadName(val) {
  ...
  if (api_response.response.code == 200) {
    ...
    table_avatar.innerHTML = `<p align="center">User <b style="font-size: 1rem !important">${val}</b> <i>(${
      User['id']
    })</i> is unreachable</p>`;
    table_name.innerHTML = '';
    images_table.innerHTML = '';
...
var messages_per_userCHART = new Chart(myChart, {
    type: 'pie',
    data: {
      labels: Names,
      datasets: [
    ...
    options: {
      title: { display: !1, text: 'Messages per user', fontSize: 25 },
      legend: {
        display: !0,
        position: 'bottom',
        labels: { fontColor: '#ffffff' },
      },
      layout: { padding: { left: 0, right: 0, bottom: 50, top: 20 } },
      tooltips: { enabled: !0 },
    },
  }),
  words_per_userCHART = new Chart(myChart1, {
    type: 'pie',
    data: {
      labels: Names2,
      datasets: [
  ...
  messages_per_channelCHART = new Chart(myChart2, {
    type: 'doughnut',
    data: {
      labels: Channels,
      datasets: [
    ...
    options: {
      title: { display: !1, text: 'Messages per Channel', fontSize: 25 },
      legend: {
        display: !0,
        position: 'right',
        labels: { fontColor: '#ffffff' },
      },
      layout: { padding: { left: 0, right: 0, bottom: 50, top: 50 } },
      tooltips: { enabled: !0 },
    },
  }),
  historicCHART = new Chart(myChart3, {
    type: 'line',
    data: {
      labels: Dates,
      datasets: [
    ...
    options: {
      ...
      title: { display: !1, text: 'Messages per Day', fontSize: 25 },
      legend: {
        display: !1,
        position: 'right',
        labels: { fontColor: '#ffffff' },
  ...
  message_freqCHART = new Chart(myChart5, {
    type: 'bar',
    data: {
      labels: Words,
      datasets: [
    ...
    options: {
      ...
      title: { display: !1, text: 'Word Frequency', fontSize: 25 },
      legend: {
        display: !1,
        position: 'right',
        labels: { fontColor: '#ffffff' },
      },
      layout: { padding: { left: 0, right: 0, bottom: 0, top: 50 } },
      tooltips: { enabled: !0 },
    },
  }),
  mentions_per_userCHART = new Chart(myChart6, {
    type: 'pie',
    data: {
      labels: Pinged,
      datasets: [
